...
export const BankRrnOverview: React.FC<BankRrnOverviewProps> = ({ user, onStart }) => {
  const archive = useMemo<ReconciliationArchive[]>(() => getArchiveData(), []);
  const latest = archive[0];
  const totalTx = archive.reduce((s, a) => s + a.matched_count + a.mismatch_count + a.only_our_count + a.only_bank_count, 0);
  const totalIssues = archive.reduce((s, a) => s + a.mismatch_count + a.only_our_count + a.only_bank_count, 0);
  const latestCount = latest ? latest.matched_count + latest.mismatch_count + latest.only_our_count + latest.only_bank_count : 0;
  const latestMatch = latestCount ? latest.matched_count / latestCount * 100 : 0;
  const runs = archive.slice(0, 5);
  const metrics = [
    { label: 'Совпадение', value: latest ? `${latestMatch.toFixed(1)}%` : '—', icon: CheckCircle2, note: latest ? 'последний запуск' : 'нет запусков', cls: 'text-emerald-600 bg-emerald-50 border-emerald-100' },
  ...
  return <div className="space-y-5">
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      ...
      <button onClick={onStart} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold shadow-sm transition-colors"><Play className="w-3.5 h-3.5 fill-white" /> Новая сверка</button>
    </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_300px] gap-5">
      <div className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between"><div><h4 className="font-semibold text-slate-900">Последние запуски</h4><p className="text-xs text-slate-500 mt-0.5">История последних операций этой сверки</p></div><span className="text-[11px] text-slate-400">{runs.length} из {archive.length}</span></div>{runs.length === 0 ? <div className="p-10 text-center text-sm text-slate-500">Сохранённых запусков пока нет. Начните новую сверку.</div> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 border-b border-slate-100"><tr>{['Дата','Банк','Оборот','Match','Статус'].map(h => <th key={h} className="text-left px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{runs.map(r => { const c = r.matched_count+r.mismatch_count+r.only_our_count+r.only_bank_count; const rate = c ? r.matched_count/c*100 : 0; const warning = r.difference !== 0 || r.mismatch_count > 0 || r.only_our_count > 0 || r.only_bank_count > 0; return <tr key={r.id} className="hover:bg-slate-50/80 transition-colors"><td className="px-5 py-3.5 font-medium text-slate-800 whitespace-nowrap">{new Date(r.timestamp).toLocaleString('ru-RU')}</td><td className="px-5 py-3.5 text-slate-700">{r.bank_name}</td><td className="px-5 py-3.5 text-slate-600">{compact(r.total_bank)}</td><td className="px-5 py-3.5 font-semibold text-slate-800">{rate.toFixed(1)}%</td><td className="px-5 py-3.5">{warning ? <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-2.5 py-1">Требует внимания</span> : <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-1">Успешно</span>}</td></tr>; })}</tbody></table></div>}</div>
      <div className="bg-slate-900 text-white rounded-xl p-5 shadow-xs"><div className="flex items-center gap-2 text-indigo-200 text-xs font-semibold uppercase tracking-wider"><ShieldCheck className="w-4 h-4" /> Контекст модуля</div><h4 className="text-lg font-bold mt-3">Сверка банков</h4><p className="text-xs text-slate-300 leading-relaxed mt-2">Рабочая область эквайринга: RRN, комиссии, терминалы и контроль расхождений.</p><div className="mt-5 space-y-2 text-xs"><div className="flex items-center justify-between py-2 border-b border-white/10"><span className="text-slate-400">Версия</span><span>1.4.2</span></div><div className="flex items-center justify-between py-2 border-b border-white/10"><span className="text-slate-400">Сохранённых запусков</span><span>{archive.length}</span></div><div className="flex items-center justify-between py-2"><span className="text-slate-400">Пользователь</span><span>{user.username}</span></div></div></div>
    </div>
  </div>;
};
